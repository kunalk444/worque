...
async function handleTaskUnArchiving(taskId){
...
function HandleNotificationClick(props){
    const {taskId,type,stopShow,showTask}=props;
    const [msg,setMsg]=useState();
    if(type!=='assigned_subtask'){
        (async()=>{
            const res=await handleTaskUnArchiving(taskId);
            console.log(res);
            if(res.success)setMsg("Task Successfully Unarchived!");
        })();
    }
    return (
        <>
