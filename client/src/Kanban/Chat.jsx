...
function Chat(props) {
  ...
  useEffect(() => {
    if (props.show) {
      ...
      socket.on("voice-message-client",(obj)=>{
  ...
  async function handleVoiceMessage() {
    if (!recorder.current) recorder.current = await voiceRecorder((blobchunk) => chunks.current.push(blobchunk));
  ...
  const handleVoiceMessageStop = async () => {
    ...
    socket && socket.emit("voice-message", {
      roomId: task._id,
      name: user.uname,
      audio: buffer,
    });
  ...
  return createPortal(
    <div className="fixed inset-0 bg-[#1e293b]/60 backdrop-blur-sm flex justify-center items-center z-[99999]">
      <div className="relative bg-[#f1f5f9]/95 rounded-2xl p-10 max-w-4xl w-full shadow-2xl h-[90vh] overflow-y-auto border-l-6 border-[#2dd4bf]">
        ...
        <div className="absolute inset-0 bg-[url('data:image/svg+xml,%3Csvg width=%2220%22 height=%2220%22 viewBox=%220 0 20 20%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cpath d=%22M0 0h20v20H0z%22 fill=%22none%22/%3E%3Ccircle cx=%221%22 cy=%221%22 r=%221%22 fill=%22rgba(45,212,191,0.05)%22/%3E%3Ccircle cx=%229%22 cy=%229%22 r=%221%22 fill=%22rgba(45,212,191,0.05)%22/%3E%3Ccircle cx=%2219%22 cy=%2219%22 r=%221%22 fill=%22rgba(45,212,191,0.05)%22/%3E%3C/svg%3E')] opacity-30 z-0"></div>
        ...
        <div className="relative z-10">
          ...
          <button
            ...
            className="absolute top-5 right-5 text-2-2xl font-bold text-[#1e293b] hover:text-[#f87171] 
                       transition-all duration-300 bg-white/70 rounded-full w-10 h-10 flex items-center 
                       justify-center hover:bg-[#2dd4bf]/20 hover:shadow-md"
          >
            X
          </button>
          ...
          <div className="mb-4">
            <h3 className="text-lg font-semibold text-[#1e293b] mb-1">
              Online Members: <span className="text-[#2dd4bf] font-bold">{online}</span>
            </h3>

            {isTyping && (
              <h2 className="text-sm italic text-[#1e293b]/80 bg-white/50 px-3 py-1 rounded-lg shadow-sm w-fit">
                {isTyping}
              </h2>
            )}
          </div>
          ...
          <div className="flex flex-col gap-4 mb-8 h-[60vh] overflow-y-auto pr-2">
            {messages.map((element, index) =>
              element.type === "audio" ? (
                ...
                >
                  <p className="font-semibold">{element.user === user.uname ? "You" : element.user}</p>
                  <audio controls src={element.url} className="mt-2 w-full"></audio>
                  <p className="text-xs mt-1">{element.time}</p>
                </div>
              ) : (
                <div
                  key={index}
          ...
          <div className="flex gap-3 items-center">
            <button 
              ...
              className="bg-[#2dd4bf] text-[#1e293b] px-4 py-2 rounded-xl hover:bg-[#f87171] 
            ...
            <button 
              ...
              className="bg-[#f87171] text-white px-4 py-2 rounded-xl hover:bg-[#e11d48] 
            ...
            <input
              ...
              className="flex-1 border border-[#2dd4bf]/40 rounded-lg px-4 py-3 
                         focus:outline-none focus:ring-2 focus:ring-[#2dd4bf] 
                         bg-[#f1f5f9] text-[#1e293b]"
            />

            <button
              onClick={handleClientMessage}
              className="bg-[#2dd4bf] hover:bg-[#f87171] text-[#1e293b] hover:text-white 
          ...
          {taskMembers.length > 0 && (
            <div className="absolute bottom-24 left-10 right-10 bg-white rounded-lg shadow-lg 
                            border border-[#2dd4bf]/30 max-h-40 overflow-y-auto z-20">
              <ul className="p-2">
                {taskMembers.map((member) => (
                  <li
                    ...
                    className="px-4 py-2 hover:bg-[#2dd4bf]/10 cursor-pointer 
                               text-[#1e293b] font-medium rounded-lg"
                  >
                    {member}
                  </li>
                ))}
